...
// ===== Web push: show "Dad sent you a message" when the Lab is closed =====
// The chat-notify Edge Function sends a push (title/body/url) whenever Dad posts.
// iOS only delivers these when the Lab has been added to the Home Screen.
self.addEventListener("push", function (event) {
	...
	var title = data.title || "Maya's Game Lab";
	var options = {
		body: data.body || "",
		icon: "icons/icon-192.png",
		badge: "icons/icon-192.png",
		tag: "maya-family-chat", // collapse repeats into one
		renotify: true,
		data: { url: data.url || "index.html?openChat=1" },
	};
	event.waitUntil(self.registration.showNotification(title, options));
});

// Tapping the notification focuses the Lab (or opens it) and asks it to open chat.
self.addEventListener("notificationclick", function (event) {
	event.notification.close();
	var target = resolve(
		(event.notification.data && event.notification.data.url) ||
			"index.html?openChat=1",
		SCOPE_URL
	);
	event.waitUntil(
		self.clients
			...
			.then(function (clientList) {
				for (var i = 0; i < clientList.length; i++) {
					var c = clientList[i];
					if (c.url.indexOf(new URL(SCOPE_URL).pathname) !== -1 && "focus" in c) {
						c.postMessage({ type: "maya:open-chat" });
						return c.focus();
					}
				}
