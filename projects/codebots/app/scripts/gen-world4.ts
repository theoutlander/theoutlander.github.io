...
const missions: Build[] = [
  // ── L1 — define + call ──────────────────────────────────────────────────────
  {
    ...
    hints: [
      "Look at the staircase: the same little move repeats — forward, turn, forward, turn back.",
      "Give that move a name with function stair() { … }, then call stair() to use it.",
      "You'll call stair() a few times to climb all the way. How many steps is the staircase?",
    ],
  ...
  // ── L2 — call your function inside a loop ───────────────────────────────────
  {
    ...
    hints: [
      "Make the stair() move again — then don't call it five times by hand.",
      "A loop can call YOUR command too: repeat n { stair() }.",
      "Count the steps in this staircase to pick the number for the loop.",
    ],
  ...
  // ── L3 — two functions ──────────────────────────────────────────────────────
  {
    ...
    hints: [
      "Two different moves here: a straight dash, and a climb up a level.",
      "Make dash() and climb() as separate functions, then use them in order.",
      "The path is dash, climb, dash — call your two commands in that sequence.",
    ],
  ...
  // ── L4 — a function that contains a loop ────────────────────────────────────
  {
    ...
    hints: [
      "A function can hold a loop inside it. row() could roll a whole row by itself.",
      "Put a repeat inside your function: function row() { repeat 4 { … } }.",
      "Then it's just row(), a turn, row() — an L in three short lines.",
    ],
  ...
  // ── L5 — functions calling functions (composition) ──────────────────────────
  {
    ...
    hints: [
      "Small tools can build bigger ones. step() moves one square; leg() is built from step().",
      "function leg() { step(); step(); step(); right() } — a function calling another function.",
      "Walk three legs to trace three sides. Wrap the legs in a repeat.",
    ],
  ...
  // ── L6 — BOSS: build a routine, solve the factory ───────────────────────────
  {
    ...
    hints: [
      "Name your staircase move once — you'll use it in two different spots.",
      "Build stair(), then use it before the dash and again after it.",
      "Repeat some stairs, forward across the gap, then repeat a few more stairs to the chest.",
    ],
